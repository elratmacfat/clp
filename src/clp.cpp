...
namespace clp {
	...
	:	Desc(szName,szWhat)
	{
		// Because the command itself does have <_parameters> just like the <OptDesc> does,
		// we treat the command and its parameters as the first option.
		// The commands' <_parameters> go to vOptDesc[0].
		// The passed <_options> start at vOptDesc[1].
		//
		...
		for( unsigned i{1}; i < m_vOptDesc.size(); i++ )
			assertm(
			   !(!m_vOptDesc[i].isOptional() && m_vOptDesc[i-1].isOptional()),
			   "Mandatory options must be declared before any optional options.");
	}
	...
	bool CmdDesc::add( const OptDesc& optDesc )
	{
		...
		if ( n )
			assertm(
			   !(!m_vOptDesc[n].isOptional() && m_vOptDesc[n-1].isOptional()),
			   "Mandatory options must be declared before any optional options.");
		return true;
	}
	...
	OptDesc& CmdDesc::operator[]( const std::string& szName )
	{
		for( OptDesc& optDesc : m_vOptDesc )
			if ( optDesc.getName() == szName )
				return optDesc;
		throw std::out_of_range("OptDesc& CmdDesc::operator[](const std::string&)");
	}
	...
	const OptDesc& CmdDesc::getOptDesc( const std::string& szName ) const
	{
		for( const OptDesc& optDesc : m_vOptDesc )
			if ( optDesc.getName() == szName )
				return optDesc;
		throw std::out_of_range("const OptDesc& CmdDesc::getOptDesc(const std::string&) const");
	}
	...
	const OptDesc& CmdDesc::getOptDesc( unsigned index ) const
	{
		if ( index < m_vOptDesc.size() )
			return m_vOptDesc[index];
		throw std::out_of_range("const OptDesc& CmdDesc::getOptDesc(unsigned) const");
	}
	...
	ErrorCode CmdDesc::validate( CmdStr& cmdStr ) const
	{
		...
		// comparing the command string (the very first word of the input line).
		...
		// Check if the bOptionaluired amount of options have been passed to the command.
		int mandatory{0};
		for ( unsigned i{1}; i < m_vOptDesc.size(); i++ )
		...
		// Let each <OptDesc> check if they find themselves satisfied in <CmdStr>
		// covered cases are: INVALID_OPTION, TOOMANY_OPT, INVALID_ARG, TOOMANY_ARG
		for ( const OptDesc& optDesc : m_vOptDesc ) {

			int optIndex{ cmdStr.getOptionIndex( optDesc.getName(), optDesc.getShortName()) };
			if ( optIndex >= 0 ) {
				// The option may have been passed by its shorthand form. Overwrite to
				// make sure every option in the CmdStr-object is given in its long version.
				cmdStr.adjustOptionName( optIndex, optDesc.getName() );
				// Pass the CmdStr option's parameters to the Option Descriptor to let
				// it validate them.
		...
		// See if there're options provided, that do not match any of the Command Descriptor's
		int cmdStrOptCount{ cmdStr.getOptionCount() };
		for( int i{0}; i < cmdStrOptCount; i++ ) {
			bool found{false};
			for( const OptDesc& optDesc : m_vOptDesc ) {
				const std::string& szOpt{ cmdStr.getOptionName(i) };
				if ( szOpt == optDesc.getName() || szOpt == optDesc.getShortName() ) {
					found=true;
					break;
				}
			}
	...
	unsigned CmdStr::parseFrom( const std::string& szSource )
	{
		...
		auto itBegin = std::sregex_iterator( szSource.begin(), szSource.end(), separating_regex );
		auto itEnd = std::sregex_iterator();
		for ( auto it = itBegin; it != itEnd; it++ )
			vElements.push_back( it->str() );
		unsigned iOption{0};
		m_vData.push_back( std::make_pair( vElements[0], std::vector<std::string>() ));
		for( unsigned i{1}; i < vElements.size(); i++ ) {
			...
			if ( isOption ) {
				m_vData.push_back( std::make_pair( vElements[i], std::vector<std::string>() ));
				iOption++;
			}
			else {
	...
	int CmdStr::getOptionIndex( const std::string& szOpt, const std::string& szShort ) const
	{
		int iSize{ static_cast<int>(m_vData.size()) };
		for( int index{0}; index < iSize ; index++ )
			if ( m_vData[index].first == szOpt || m_vData[index].first == szShort )
				return index;
		return -1;
	}
	...
	void printCmdDesc( const CmdDesc& cmdDesc, bool bLongVersion, std::ostream& os )
	{
		if ( bLongVersion ) {
			...
			for( unsigned j{0}; j < nOptDesc; j++ ) {
				...
				if ( j == 0 ) {
					os << indent(0) << cmdDesc.getName() << " - " << cmdDesc.getWhat() << "\n";
					os << indent(1) << "Syntax: ";
					...
					if ( s.length() )
						os << "|-" << s;
					os << ( optDesc.isOptional() ? " - Optional. " : " - " )
					   << optDesc.getWhat() << "\n";
				}
				unsigned nParamDesc{ optDesc.getParamCount() };
				for( unsigned k{0}; k < nParamDesc; k++ ) {
					const std::shared_ptr<ParamDesc>& pParamDesc{ optDesc.getParamDesc(k) };
					os << ( j ? indent(2) : indent(1) )
					   << "<" << pParamDesc->getName() << ( pParamDesc->isOptional() ? "> - Optional. " : "> - " )
					   << pParamDesc->getWhat() << " ";
			...
			for( unsigned j{0}; j < nOptDesc; j++ ) {
				...
				for( unsigned k{0}; k < nParamDesc; k++ ) {
					const std::shared_ptr<ParamDesc>& pParamDesc{ optDesc.getParamDesc(k) };
					os	<< ( pParamDesc->isOptional() ? "[" : "" ) << "<" << pParamDesc->getName();
					printConstraints( pParamDesc->getConstraints(), os );
					os	<< ">"
						<< ( pParamDesc->isOptional() ? "]" : "" );
				} // k

				os << ( optDesc.isOptional() && nParamDesc ? "] " : " ");	 // close optional-bracket after parameter list
			} // j
			os << "\n";
		}
	}
